...
MMsimu=function(m,x,z,tt,nj,beta,sigmae,D,phi,struc,typeModel,percCensu,cens.type,nu){
 Arp=length(phi)
 rx="NO";rz="NO"
  ...
  for(i in 1:m){
    ...
    if(struc!="ARp"){
      for(i in 1:m){
        ...
        Omegai <- sigmae*ome2
        errorp=as.vector(rmvnorm(1,rep(0,n),  Omegai ))
        b<-rmvnorm(1,rep(0,dim(D)[1]), D)
        y[(sum(nj[1:i-1])+1) : (sum(nj[1:i])),]=x[(sum(nj[1:i-1])+1) : (sum(nj[1:i])),]%*%beta+z[(sum(nj[1:i-1])+1) : (sum(nj[1:i])),]%*%t(b)+errorp
      }
    } 
  ...
  for(i in 1:m)
  {
    ...
    ome2=MatArp(phi,tt1,sigmae)
    errorp=as.vector(rmvnorm(1,rep(0,n), ome2))
    b <-as.matrix(rmvt(n = 1,sigma =D,df = nu,delta =  c(0,0)),q,1)
    y[(sum(nj[1:i-1])+1) : (sum(nj[1:i])),]=x[(sum(nj[1:i-1])+1) : (sum(nj[1:i])),]%*%beta+z[(sum(nj[1:i-1])+1) : (sum(nj[1:i])),]%*%t(b)+errorp
  }}
    if(struc!="ARp"){
      for(i in 1:m)
      {
        ...
        Omegai <- sigmae*ome2
        b <-as.matrix(rmvt(n = 1,sigma =D,df = nu,delta =  c(0,0)),q,1)
        errorp <- as.matrix(rmvt(n = 1,sigma =Omegai,df = nu,delta =  rep(0,nj[i])),nj[i],1)
        y[(sum(nj[1:i-1])+1) : (sum(nj[1:i])),]=x[(sum(nj[1:i-1])+1) : (sum(nj[1:i])),]%*%beta+z[(sum(nj[1:i-1])+1) : (sum(nj[1:i])),]%*%t(b)+errorp
      }}
  }
